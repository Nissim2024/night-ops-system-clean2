# DeployCenter 2.7.5 — מדריך התקנה ושדרוג
...
```
release-v2.7.5/
├── deploycenter-docker-v2.7.5.tar      ← Docker images (API + Frontend + PostgreSQL)
...
- גיבוי DB לפני שדרוג: `/opt/deploycenter/backup-pre-2.7.5-DATE.sql`
- גיבוי קונפיגורציה: `/opt/deploycenter/config-backup-pre-2.7.5-DATE.env`
- עצירת containers קיימים (`docker compose down`) — מונע שגיאת name conflict
- **אין migrations חדשות וללא משתני `.env` חדשים בגרסה זו** — 2.7.5 הוא release של תיקוני קוד/UX בלבד, אין שינוי סכימת DB
...
sleep 10
cat /opt/deploycenter/backup-pre-2.7.5-DATE.sql | docker exec -i dc-postgres \
