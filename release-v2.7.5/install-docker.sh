#!/bin/bash
# ============================================================
# DeployCenter 2.7.5 — התקנה / שדרוג (ללא אינטרנט)
# תומך ב: RHEL 9.7 + Docker + Oracle 11g + SMB/CIFS
#
# שדרוג מ-2.7.x (אותו שרת):
#   1. העבר: deploycenter-docker-v2.7.5.tar + docker-compose.offline.yml
...
#
# התקנה חדשה (שרת ריק):
#   1. העבר את כל הקבצים מתיקיית release-v2.7.5
...
if [ "$IS_UPGRADE" = "true" ]; then
  ...
  echo ""
  echo "בודק שכל משתני הסביבה הנדרשים (מצטבר עד 2.7.5) קיימים ב-.env..."
