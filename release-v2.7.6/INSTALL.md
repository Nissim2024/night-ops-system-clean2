...
- **5 migrations חדשות בגרסה זו, ללא משתני `.env` חדשים** — כולן `ADD COLUMN` תוספתיות (nullable / עם default), לא הורסות נתונים קיימים: `TesterSkill.lastUsedAt`, `QaAssignment.standAloneDueDate`, `QaAssignment.secondaryParticipationPct`, `QaWorkPlan.cycle1LengthDays`, `QaWorkPlan.cycle2LengthDays`/`cycle3LengthDays`. מוחלות אוטומטית ע"י `prisma migrate deploy` בעת עליית ה-container (`docker-entrypoint.sh`) — אין פעולה ידנית נדרשת.
...
*תכנון ושיבוץ QA — עיצוב מחדש של מנוע התזמון:*
- אורך כל סבב (1/2/3) הוא כעת פרמטר קבוע וניתן לעריכה בנפרד לכל סבב (במקום "להימתח" לפי הבודק האיטי ביותר) — בודקים שלא מספיקים בתוך הסבב שלהם מסומנים כחורגים (CORE_OVERFLOW) במקום לגרור את כל הצוות לסבב הבא.
- משימות Stand Alone (SA) עצמאיות ממועד תחילת סבב 1 — יכולות להתחיל לפני שהגרסה בכלל נפתחת.
- מילוי משימות רגרסיה רץ כעת רק אחרי שגם משימות הליבה וגם משימות ה-SA שובצו (סדר עדיפות: CR > SA > רגרסיה).
- סבבי UAT מייצרים כעת משימות אמיתיות (0.5 יום, תלוי-CR עם תגית UAT) הממוקמות במקביל לימי הסיום של סבב 1, במקום פלייסהולדר ריק אחרי החזרה.
- נוספה בדיקת חריגה חדשה: משימות ליבה שחורגות ממועד העלייה לאוויר (`Version.plannedStart`) מסומנות GO_LIVE_OVERFLOW.
- עמודת "ימי בדיקות (פיצול)" חדשה בתוכנית העבודה, וסימון/קישור-קפיצה לשורה הבעייתית ישירות מהודעת החריגה.

*בודק שני — זמין כעת גם ממסך השיבוץ (לא רק מתוכנית העבודה):*
- אחוז השתתפות ניתן לעריכה (`secondaryParticipationPct`, ברירת מחדל 50%), עם עומס מחושב בהתאם לכל אחד מהבודקים.
- 4 תיקוני באגים: עומס לא התעדכן בהוספת בודק שני; עריכת % ואז לחיצה במקום אחר מחקה את הבודק השני (חסר `secondaryUser` ב-include של ה-API); שדה ה-% היה נחתך ויזואלית; לא הייתה דרך למחוק רק את הבודק השני בלי למחוק גם את הראשי.
- מסך השיבוץ ומסך תוכנית העבודה מסתנכרנים כעת אוטומטית במעבר בין טאבים (ללא צורך ברענון ידני).

*לוח באגי QC (חדש):* מסך MVP להצגת באגים מ-QC לפי גרסה/צוות.

*שונות:* `TesterSkill.lastUsedAt` — שדה חדש שמזין את ציון ה"רציפות" של מנוע השיבוץ האוטומטי.

> **שדרוג בטוח מבחינת נתונים:** הייבוא הוא upsert-בלבד — לא מוחק שום דבר. משתמשים/צוותים/הרשאות/סקילים קיימים בשרת היעד נשמרים ורק מתעדכנים/מתווספים. ה-migrations החדשות הן תוספתיות בלבד (עמודות nullable/עם default) ולא נוגעות בנתונים קיימים.
>
> **בדיקה לאחר שדרוג:** ודא שמסך "שיבוץ ותוכנית עבודה" (QA) עולה תקין, שניתן להוסיף/להסיר בודק שני, ושתוכנית עבודה קיימת מציגה את אורכי הסבבים כצפוי.
...
docker load -i deploycenter-docker-v2.7.5.tar
