...
export class VersionCrAssignmentsController {
  ...
  // Version-management module landing page — tile row + developments treemap.
  @Get('version/:versionId/scope-overview')
  getScopeOverview(@Param('versionId') versionId: string, @Request() req: any) {
    if (!LEADS_UP.includes(req.user.role)) throw new ForbiddenException('נדרשת הרשאת ראש צוות לפחות');
