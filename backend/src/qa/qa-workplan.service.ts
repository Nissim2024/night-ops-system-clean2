...
// QG Threshold High/Medium/Low + Environment defaults per cycle type
// (2026-09-23, fixes-batch) — QC's real release-cycle entity requires these
// at creation time (user-01 = QG Threshold High is mandatory; discovered the
// hard way when publishVersionRelease's "צור ב-QC" failed with
// qccore.required-field-missing against real production QC). Values
// confirmed against the user's own real QC RELEASE_CYCLES table export
// (2026-09-23) — consistent across every release sampled for every cycle
// type DeployCenter actually creates. Note: QC also has a "Cycle 0" type in
// that export (100/100/100, "3 - Integration") that DeployCenter has no
// local equivalent for — never generated by our own scheduler, so it's
// deliberately absent here. Environment values come from the same real QC
// picklist (List-Id 172) already cached for the defect edit form — see
// QcRestService.FIELD_LIST_ID's `environment` entry / QcPicklistCache.
export const QG_CYCLE_DEFAULTS: Record<CycleType, { high: number; medium: number; low: number; environment: string }> = {
  CYCLE_1:     { high: 90,  medium: 85,  low: 75,  environment: '2 - Test' },
  CYCLE_2:     { high: 95,  medium: 90,  low: 85,  environment: '2 - Test' },
  CYCLE_3:     { high: 98,  medium: 98,  low: 98,  environment: '2 - Test' },
  STAND_ALONE: { high: 100, medium: 100, low: 100, environment: '2 - Test' },
  UAT:         { high: 100, medium: 100, low: 100, environment: '2 - Test' },
  REHEARSAL:   { high: 98,  medium: 85,  low: 75,  environment: '3 - Integration' },
  GO_LIVE:     { high: 100, medium: 100, low: 100, environment: '4 - Production' },
};
...
export class QaWorkPlanService {
  ...
  async generateWorkPlan(
    ...
    await prisma.$transaction(async tx => {
      ...
      for (let ci = 0; ci < plannedCycles.length; ci++) {
        ...
        const qgDefaults = QG_CYCLE_DEFAULTS[pc.cycleType];
        const cycle = await tx.qaCycle.create({
          data: {
            ...
            qgThresholdHigh:   qgDefaults?.high ?? null,
            qgThresholdMedium: qgDefaults?.medium ?? null,
            qgThresholdLow:    qgDefaults?.low ?? null,
            qgEnvironment:     qgDefaults?.environment ?? null,
          },
        });
  ...
  // override of QG_CYCLE_DEFAULTS before "צור ב-QC" is clicked; publish reads
  // whatever is stored on the row (default or edited) at publish time. ──────
  async updateCycleQg(cycleId: string, data: {
    qgThresholdHigh?: number | null; qgThresholdMedium?: number | null;
    qgThresholdLow?: number | null; qgEnvironment?: string | null;
  }) {
    return prisma.qaCycle.update({ where: { id: cycleId }, data });
  }
