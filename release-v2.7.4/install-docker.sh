#!/bin/bash
# ============================================================
# DeployCenter 2.7.4 — התקנה / שדרוג (ללא אינטרנט)
# תומך ב: RHEL 9.7 + Docker + Oracle 11g + SMB/CIFS
#
# שדרוג מ-2.7.0 / 2.7.1 / 2.7.2 (אותו שרת):
#   1. העבר: deploycenter-docker-v2.7.4.tar + docker-compose.offline.yml
...
#
# התקנה חדשה (שרת ריק):
#   1. העבר את כל הקבצים מתיקיית release-v2.7.4
...
if [ "$IS_UPGRADE" = "true" ]; then
  ...
  # ── [#10] שמירת קונפיגורציה קיימת לפני שדרוג ─────────────
  ...
  echo ""
  echo "בודק שכל משתני הסביבה הנדרשים (מצטבר עד 2.7.4) קיימים ב-.env..."
  ...
  # ── [#10] השבת ערכי קונפיגורציה שאבדו ──────────────────
  echo ""
...
# ── [#1] עצור containers קיימים לפני עדכון ───────────
echo ""
cd "$APP_DIR"
if [ "$IS_UPGRADE" = "true" ]; then
...
# ── [#10] וידוא שמירת קונפיגורציה Oracle ─────────────
...
if [ "$IS_UPGRADE" = "true" ]; then
  ...
  echo "   ✅ LD_LIBRARY_PATH + ORACLE_HOME נוספו ל-.env"
