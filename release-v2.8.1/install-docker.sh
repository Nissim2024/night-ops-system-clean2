#!/bin/bash
# ============================================================
# DeployCenter 2.8.1 — התקנה / שדרוג (ללא אינטרנט)
# תומך ב: RHEL 9.7 + Docker + Oracle 11g + SMB/CIFS
#
# שדרוג מ-2.8.x (אותו שרת):
#   1. העבר: deploycenter-docker-v2.8.1.tar + docker-compose.offline.yml
...
#
# התקנה חדשה (שרת ריק):
#   1. העבר את כל הקבצים מתיקיית release-v2.8.1
...
if [ "$IS_UPGRADE" = "true" ]; then
  ...
  echo ""
  echo "בודק שכל משתני הסביבה הנדרשים (מצטבר עד 2.8.1) קיימים ב-.env..."
